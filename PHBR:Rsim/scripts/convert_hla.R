...
# Check if the file has only a header or is completely empty
if (nrow(MHC_gt) == 0) {
  message("⚠️ Input CSV has no data — skipping processing.")
...
mhc1_all <- unlist(MHC_gt[, c("A.1", "A.2", "B.1", "B.2", "C.1", "C.2")])
mhc2_all <- unlist(lapply(1:nrow(MHC_gt), function(i) c(
