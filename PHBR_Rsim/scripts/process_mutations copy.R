...
input_file <- args[1]
output_file <- args[2]
uuid_to_tcga_file <- args[3]
sample_id <- args[4]
mapping_file <- args[5]
mane_file <- args[6]

# === Load UUID → TCGA Mapping ===
uuid_to_tcga <- read.csv(uuid_to_tcga_file, stringsAsFactors = FALSE)
...
if (nrow(peptides_df) > 0) {
  ...
  uuid <- stringr::str_extract(sample_id, "[a-f0-9\\-]{36}")

  if (!is.na(uuid) && uuid %in% uuid_to_tcga$uuid) {
    tcga_id <- uuid_to_tcga$tcga_id[uuid_to_tcga$uuid == uuid]
  } else {
    message("⚠️ UUID not found — using sample_id instead")
    tcga_id <- sample_id
  }

  peptides_df$sample_id <- tcga_id

  write.table(peptides_df, file = output_file, sep = "\t", row.names = FALSE, quote = FALSE)
  message("✅ Peptides written to: ", output_file)
} else {
